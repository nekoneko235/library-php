...
class MaxSparseTable
{
    ...
    public function __construct(array $values)
    {
        ...
        for ($p = 1; $p <= $this->P; $p++) {
            for ($i = 0; $i + (1 << $p) <= $this->n; $i++) {
                ...
                $this->dp[$p][$i] = max($leftInterval, $rightInterval);
    ...
    public function queryMax(int $l, int $r): int
    {
        ...
        return max($this->dp[$p][$l], $this->dp[$p][$r - $k + 1]);
    }

    // Returns the index of the maximum element in the range [l, r].
    public function queryMaxIndex(int $l, int $r): int
    {
        $length = $r - $l + 1;
        $p = $this->log2[$length];
        $k = 1 << $p; // 2 to the power of p
    ...
    {
        // index:  0, 1,  2, 3, 4, 5,  6
        $values = [3, 1, -4, 2, 1, 5, -1];
        $maxSparseTable = new MaxSparseTable($values);

        echo $maxSparseTable->queryMax(0, 6) . "\n";
        echo $maxSparseTable->queryMaxIndex(0, 6) . "\n";
